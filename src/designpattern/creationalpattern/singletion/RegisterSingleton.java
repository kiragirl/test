...
/**
 * Title: RegisterSingleton Description: Company:pusense 5、登记式/静态内部类
 * 
 * 是否 Lazy 初始化：是
 * 
 * 是否多线程安全：是
 ...
 * 双检锁方式可在实例域需要延迟初始化时使用。 这种方式同样利用了 classloder 机制来保证初始化 instance 时只有一个线程，它跟第 3
 * 种方式不同的是：第 3 种方式只要 Singleton 类被装载了，那么 instance 就会被实例化（没有达到 lazy loading
